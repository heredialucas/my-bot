...
export default function PlanForm({ plan }: PlanFormProps) {
    ...
    // Inicializar las características dinámicas con las del plan existente
    const [planCharacteristics, setPlanCharacteristics] = useState<Array<{
        key: string;
        value: boolean;
    }>>(
        // Si el plan tiene características, usarlas
        plan.characteristics && plan.characteristics.length > 0
            ? plan.characteristics.map(char => ({ key: char.key, value: char.value }))
            // Si no, iniciar con una característica vacía
            : [{ key: '', value: true }]
    );
    ...
    const previewRegularPrice = regularPrice ? parseFloat(regularPrice) : undefined;
    const previewChannelCount = channelCount ? parseInt(channelCount) : undefined;
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Ahora usamos directamente las características dinámicas
            ...
            <div className="bg-gray-900 text-white flex-1 p-3">
                ...
                <div className="mt-2">
                    ...
                    <div className="ml-6 mb-2">
                        <p className="text-xl font-bold">${previewPrice.toLocaleString('es-CL')}<span className="text-xs font-normal ml-1">primer mes</span></p>
                        <p className="text-xs text-gray-400">luego ${(previewRegularPrice || previewPrice + 2000).toLocaleString('es-CL')} /mes</p>
                    </div>
                    ...
                    <div className="ml-6 mb-2">
                        ...
                        <div className="flex items-center">
                            <p className="text-xs font-medium">Zapping con <span className="text-rose-400">+{previewChannelCount || 65} canales</span></p>
